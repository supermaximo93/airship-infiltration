...
// lib/game/entities/particleEffect.js
ig.baked=true;ig.module('game.entities.particleEffect').requires('impact.entity').defines(function(){EntityParticleEffect=ig.Entity.extend({_wmDrawBox:true,_wmBoxColor:'rgba(255, 0, 0, 0.7)',size:{x:10,y:10},lifetime:1,callback:null,particleCount:20,timer:null,init:function(x,y,settings){this.parent(x,y,settings);if(!ig.game.spawnEntity)return;for(var i=0;i<this.particleCount;i++){var particleSettings={lifetime:this.lifetime};if(settings.particle&&typeof(settings.particle)=='object'){for(var key in settings.particle){particleSettings[key]=settings.particle[key];}}
ig.game.spawnEntity(EntityParticleEffectParticle,this.pos.x,this.pos.y,particleSettings);}
this.timer=new ig.Timer();},update:function(){if(this.timer.delta()>this.lifetime){if(this.callback)this.callback();this.kill();return;}},draw:function(){}});EntityParticleEffect.create=function(x,y,settings){ig.game.spawnEntity(EntityParticleEffect,x,y,settings);}
EntityParticleEffectParticle=ig.Entity.extend({animSheet:new ig.AnimationSheet('media/particle.png',1,1),size:{x:1,y:1},maxVel:{x:160,y:200},lifetime:1,fadetime:1,bounciness:0.3,vel:{x:40,y:-30},friction:{x:20,y:20},timer:null,totalParticles:5,init:function(x,y,settings){this.parent(x,y,settings);this.vel.x=(Math.random()*4-1)*this.vel.x;this.vel.y=(Math.random()*10-1)*this.vel.y;this.timer=new ig.Timer();var frame=Math.round(Math.random()*(this.totalParticles-1));this.addAnim('idle',1,[frame]);this.currentAnim=this.anims.idle;},update:function(){if(this.timer.delta()>this.lifetime){this.kill();return;}
this.currentAnim.alpha=this.timer.delta().map(this.lifetime-this.fadetime,this.lifetime,1,0);if(this.vel.x<0){this.currentAnim.angle-=0.1;}else if(this.vel.x>0){this.currentAnim.angle+=0.1;}
this.parent();}});});

// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.font','game.entities.particleEffect','..game.levels.menu','..game.levels.ending','..game.levels.level1','..game.levels.level2','game.levels.level3').defines(function(){MainGame=ig.Game.extend({font:new ig.Font('media/04b03.font.png'),pauseBackground:new ig.Image('media/pause_background.png'),gravity:300,player:null,gameManager:null,paused:false,briefing:false,won:false,lost:false,levels:[LevelLevel1,LevelLevel2,LevelLevel3],currentLevelId:0,init:function(){ig.input.bind(ig.KEY.LEFT_ARROW,'left');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');ig.input.bind(ig.KEY.UP_ARROW,'jump');ig.input.bind(ig.KEY.Z,'jump');ig.input.bind(ig.KEY.X,'fire');ig.input.bind(ig.KEY.C,'hack');ig.input.bind(ig.KEY.SPACE,'confirm');ig.input.bind(ig.KEY.P,'pause');ig.music.add('media/music/music.*');ig.music.volume=0.5;ig.music.loop=true;ig.music.play();this.loadLevel(this.levels[this.currentLevelId]);this.paused=this.briefing=true;},update:function(){if(this.player){this.screen.x=this.player.pos.x-ig.system.width/2;this.screen.y=this.player.pos.y-ig.system.height/2;}
if(this.paused){if(this.briefing){if(ig.input.pressed('confirm'))this.briefing=this.paused=false;}else if(this.won){if(ig.input.pressed('confirm'))this.goToNextLevel();}else if(this.lost){if(ig.input.pressed('confirm'))this.restartLevel();}else{if(ig.input.pressed('pause'))this.paused=false;}}else{this.parent();if(ig.input.pressed('pause'))this.paused=true;}},draw:function(){this.parent();if(this.paused){this.pauseBackground.draw(0,0);if(this.won){this.font.draw("MISSION COMPLETE",120,80,ig.Font.ALIGN.CENTER);this.font.draw("Press SPACE to continue",7,148,ig.Font.ALIGN.LEFT);}else if(this.lost){this.font.draw("MISSION FAILED",120,80,ig.Font.ALIGN.CENTER);this.font.draw("Press SPACE to restart",7,148,ig.Font.ALIGN.LEFT);}else{this.gameManager.drawGames();}}},playerIsVisibleTo:function(entity,xRestriction,yRestriction){if(xRestriction===undefined)xRestriction=true;if(yRestriction===undefined)yRestriction=true;var playerVisible=false;if(this.player){var entityCenter=entity.center(),playerCenter=this.player.center();var xd=playerCenter.x-entityCenter.x,yd=playerCenter.y-entityCenter.y;if(xRestriction&&((entity.flip&&xd>0)||(!entity.flip&&xd<0)))return false;if(yRestriction&&((yd>entity.size.y)||(yd<-entity.size.y)))return false;var distanceToPlayerSquared=(xd*xd)+(yd*yd);if(distanceToPlayerSquared<=entity.lengthOfSightSquared){result=this.collisionMap.trace(entityCenter.x,entityCenter.y,xd,yd,1,1);return!result.collision.x&&!result.collision.y;}}
return playerVisible;},win:function(){this.won=this.paused=true;},lose:function(){this.lost=this.paused=true;},goToNextLevel:function(){if(++this.currentLevelId>=this.levels.length){ig.system.setGame(Ending);}else{this.restartLevel();}},restartLevel:function(){this.loadLevelDeferred(this.levels[this.currentLevelId]);},loadLevel:function(level){this.parent(level);this.won=this.lost=false;this.paused=this.briefing=true;}});Menu=ig.Game.extend({font:new ig.Font('media/04b03_black.font.png'),gravity:300,started:false,init:function(){ig.input.bind(ig.KEY.SPACE,'confirm');this.loadLevel(LevelMenu);},update:function(){if(ig.input.pressed('confirm')&&!this.started){this.getEntitiesByType(EntityMenuPlayer)[0].go();this.started=true;}
this.parent();},draw:function(){this.parent();if(!this.started){this.font.draw('AIRSHIP INFILTRATION',7,7,ig.Font.ALIGN.LEFT);this.font.draw('By Max Foster',7,15,ig.Font.ALIGN.LEFT);this.font.draw('LEFT/RIGHT to move',120,120,ig.Font.ALIGN.CENTER);this.font.draw('Z/UP to jump',120,128,ig.Font.ALIGN.CENTER);this.font.draw('X to shoot',120,136,ig.Font.ALIGN.CENTER);this.font.draw('PRESS SPACE TO START',120,150,ig.Font.ALIGN.CENTER);}},startGame:function(){ig.system.setGame(MainGame);}});Ending=ig.Game.extend({font:new ig.Font('media/04b03_black.font.png'),gravity:300,started:false,timer:null,init:function(){ig.input.bind(ig.KEY.SPACE,'confirm');this.loadLevel(LevelEnding);timer=new ig.Timer();},update:function(){if(timer.delta()>2){var airship=this.getEntitiesByType(EntityMenuAirship)[0];if(airship&&!airship.exploding)airship.explode();}
this.parent();},backToMenu:function(){ig.system.setGame(Menu);}});ig.Entity.inject({center:function(){return{x:this.pos.x+this.size.x/2,y:this.pos.y+this.size.y/2};}});ig.Font.inject({drawRelative:function(text,x,y,alignment){this.draw(text,x-ig.game.screen.x,y-ig.game.screen.y,alignment);}});if(ig.ua.mobile)ig.Sound.enabled=false;ig.main('#canvas',Menu,60,240,160,3);});