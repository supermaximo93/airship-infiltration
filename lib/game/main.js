...
MainGame = ig.Game.extend({
  ...
  update: function() {
    ...
    if (this.paused) {
      if (ig.input.pressed('quit')) ig.system.setGame(Menu);
      if (this.briefing) {
...
MainGame.levels = [LevelLevel1, LevelLevel2, LevelLevel3, LevelLevel4, LevelLevel5, LevelLevel6];
  ...
  init: function() {
    ...
    for (var column = 0; column < columns; column++) {
      ...
      for (var row = 0; row < rows; row++) {
        this.spawnEntity(EntityMenuLevelButton, ig.system.width - (18 * (columns - column)), row * 20 + 2, { levelId: levelId });
        levelId++;
