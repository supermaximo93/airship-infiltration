...
.defines(function() {
  EntityGameManager = ig.Entity.extend({
    ...
    afterInit: function() {
      ...
      if (this.keycardsToAssignCount > 0) {
        var guards = ig.game.getEntitiesByType(EntityGuard).reject(function(guard) {
          return guard.hasKeycard;
        });
        for (var i = 0; i < this.keycardsToAssignCount && guards.length > 0; i++) {
          var index = Math.round(Math.random() * guards.length);
          guards[index].hasKeycard = true;
