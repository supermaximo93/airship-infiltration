...
.defines(function() {
  EntityGuard = ig.Entity.extend({
    ...
    init: function(x, y, settings) {
      ...
      this.hasKeycard = this.hasKeycard === 'true';
    ...
    receiveDamage: function(amount, from) {
      ...
      if (!this.alerted && this.playerIsBehind()) {
        this.flip = !this.flip;
        this.setEmote('?');
        this.setHitScanTimer(this.hitScanTimerInterval - 0.1, function() {
          if (!this.playerIsVisible()) ig.game.player.sneaky();
        });
      }
    },
    ...
    kill: function() {
      ...
      if (this.hasKeycard) ig.game.spawnEntity(EntityKeycard, this.center().x, this.center().y, { vel: { x: 0, y: -60 } });
    ...
    playerIsVisible: function() {
      if (this.hitScanTimer.delta() > this.hitScanTimerInterval) {
        this.playerIsVisibleResult = ig.game.playerIsVisibleTo(this, true, true);
        this.hitScanTimer.reset();
        if (this.hitScanTimerCallback) {
          this.hitScanTimerCallback();
          this.hitScanTimerCallback = null;
        }
      }
      return this.playerIsVisibleResult;
    },

    playerIsBehind: function() {
      if (!ig.game.player) return false;
      var playerCenter = ig.game.player.center(), thisCenter = this.center();
      return (this.flip && playerCenter.x > thisCenter.x) || (!this.flip && playerCenter.x < thisCenter.x);
    },
    ...
    setHitScanTimer: function(time, callback) {
      this.hitScanTimerCallback = callback;
      this.hitScanTimer.set(-time);
    }
  });
