  ...
  EntityTurret = ig.Entity.extend({
    ...
    init: function(x, y, settings) {
      ...
      if (ig.game.spawnEntity) {
        gun = ig.game.spawnEntity(EntityTurretGun, x + this.size.x / 2, y + this.size.y / 2, { parentTurret: this });
      }
    },
    ...
    update: function() {
      if (this.parentTurret && this.parentTurret.canSeePlayer()) {
        ...
        if (this.fireTimer.delta() > this.fireInterval) {
          ig.game.spawnEntity(EntityTurretBullet, this.parentTurret.center().x, this.parentTurret.center().y, { angle: angle });
