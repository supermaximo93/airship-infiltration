...
.defines(function() {
  EntityTurret = ig.Entity.extend({
    ...
    init: function(x, y, settings) {
      ...
      if (ig.game.spawnEntity) {
        gun = ig.game.spawnEntity(EntityTurretGun, this.center().x, this.center().y, { parentTurret: this });
      }
    },

    kill: function() {
      EntityParticleEffect.create(this.pos.x, this.pos.y, {});
      EntityParticleEffect.create(this.pos.x, this.pos.y, {
        particle: {
          animSheet: new ig.AnimationSheet('media/turret_particle.png', 3, 3),
          totalParticles: 3,
          lifeTime: 2
        },
        particleCount: 6
    ...
    canSeePlayer: function() {
      return ig.game.playerIsVisibleTo(this, false, false);
    }
  });
  ...
  EntityTurretGun = ig.Entity.extend({
    ...
    update: function() {
      if (this.parentTurret && this.parentTurret.canSeePlayer()) {
