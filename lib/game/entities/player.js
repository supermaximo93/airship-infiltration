...
.defines(function() {
  EntityPlayer = ig.Entity.extend({
    ...
    init: function(x, y, settings) {
      ...
      if (ig.game.goalManager && ig.game.goalManager.spawnPoint === null) ig.game.goalManager.spawnPoint = { x: x, y: y };
    ...
    update: function() {
      ...
      if (ig.input.pressed('fire') && !ig.input.state('hack')) {
    ...
    draw: function() {
      this.parent();
      if (ig.input.state('hack') && ig.game.font) {
    ...
    kill: function() {
      var callback = ig.game.goalManager ? ig.game.goalManager.playerDeathTrigger.bind(ig.game.goalManager) : null;
      ...
      if (this.gold > 0) ig.game.spawnEntity(EntityGold, this.pos.x, this.pos.y, { value: this.gold });
